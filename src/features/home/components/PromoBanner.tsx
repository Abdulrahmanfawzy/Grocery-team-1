  ...
  return (
    <section className="box-container grid gap-4 py-8 sm:grid-cols-2 sm:py-10">
      <article className="relative min-h-40 overflow-hidden rounded-md bg-[#c8c5c1] px-5 py-5 sm:px-6">
        ...
        <h3 className="mt-3 max-w-60 text-xl font-bold text-app-main">Free delivery over $50</h3>
        <p className="mt-1 max-w-55 text-xs text-app-main">Shop $50 product and get free delivery anywhere.</p>
      ...
      <article className="relative min-h-40 overflow-hidden rounded-md bg-app-main px-5 py-5 text-white sm:px-6">
        ...
        <h3 className="mt-3 max-w-55 text-xl font-bold">Organic Food</h3>
        <p className="mt-1 max-w-52.5 text-xs">Save up to 60% off on your first order</p>
        <Link to="/products" className="mt-5 inline-flex rounded bg-white px-4 py-2 text-xs font-medium text-app-main">
          Shop Now <span className="ml-2">›</span>
        </Link>
        <img src={organicFruit} alt="Organic fruit" className="absolute bottom-0 right-0 h-full w-1/2 object-contain object-bottom-right" />
      </article>
    </section>
  )
}
